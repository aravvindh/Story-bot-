  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-b from-cyan-300 via-sky-400 to-indigo-500 text-gray-800 p-4 flex flex-col items-center overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        ...
        {/* Sparkles */}
        <div className="sparkle" style={{ top: '15%', left: '20%', animationDelay: '0s' }}></div>
        <div className="sparkle" style={{ top: '50%', left: '10%', animationDelay: '1s' }}></div>
        <div className="sparkle" style={{ top: '80%', left: '30%', animationDelay: '2s' }}></div>
      ...
      <main className="w-full max-w-2xl flex-grow flex flex-col z-10">
        <div className="bg-white/60 backdrop-blur-md p-6 rounded-3xl shadow-2xl flex-grow flex flex-col justify-center gap-4">
          {showWelcome && (
             <div className="text-center p-4 animate-pop-in">
                <h2 className="text-3xl font-bold text-sky-700 mb-2">Hello friend!</h2>
                <p className="text-gray-700 text-lg">I'm StoryBot! What are you curious about today? Ask me anything!</p>
             </div>
          )}
          {isLoading && (
            <div className="flex flex-col items-center justify-center gap-4 p-8">
              ...
              <p className="text-lg font-semibold text-sky-800/90 mt-4">Thinking up something wonderful for you...</p>
            </div>
          )}
          {error && (
          ...
          {apiResponse && !isLoading && (
            apiResponse.type === 'story' ? (
              <StoryDisplay story={apiResponse.data.story} example={apiResponse.data.example} />
            ) : (
              <div className="text-center p-4 animate-pop-in">
                <p className="text-2xl font-bold text-indigo-700">{apiResponse.data.greeting}</p>
              </div>
            )
          )}
        </div>
